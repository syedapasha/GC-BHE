...
#------------------------------------------
# simulate a scalar Hawkes-Laguerre process
#------------------------------------------
 
sim_sHL = function(T, cc, a, be) {
	...
	while (s<T & bProceed==1) {
		...
		# compute Riemann-Stieltjes (RS) sum of phi at event time s
		sum_phi <- sapply(1:p, function(x) sum(exp(-be*(s - pp[pp<s])) * ((be*(s - pp[pp<s]))^(x-1)) * be / gamma(x)))
		
		lam <- cc + sum(a*sum_phi)
		...
		while (s<T & bProceed==0) {
			...
			# compute the RS sum of phi at the new event time s
			sum_phi <- sapply(1:p, function(x) sum(exp(-be*(s - pp[pp<s])) * ((be*(s - pp[pp<s]))^(x-1)) * be / gamma(x)))
